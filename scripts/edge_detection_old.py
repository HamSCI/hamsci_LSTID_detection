...
def run_edge_detect(
    ...
    **kwArgs):
    ...
    arr     = np.nan_to_num(arr, nan=0) #where stops being an xarray
    print(type(arr).__name__, arr.shape, arr.dtype, Ts)
    ...
    # now do the fit
    try:
        # Curve Fit 2nd Degree Polynomial #########  
        ...
        # Generate sinusoid guesses
        T_hr_guesses = np.arange(1, 4.5, 0.5)
    
        # ==== Corrected Sinusoid Fit Loop ====
        all_sin_fits = []         # param dicts
        all_sin_fit_series = []   # actual Series
        ...
        for T_hr_guess in T_hr_guesses:
            ...
            try:
                sinFit, pcov, infodict, mesg, ier = curve_fit(
                    sinusoid, tt_sec, data_detrend, p0=list(guess.values()), full_output=True
                )
            except:
                continue
            ...
            # Create Series aligned to fit_times
            sin_fit_series = sinusoid(tt_sec, **p0_sin_fit)
            sin_fit_series = pd.Series(sin_fit_series, index=fit_times)
    
            # Ensure Series is numeric and 1D
            if isinstance(sin_fit_series, pd.DataFrame):
                sin_fit_series = sin_fit_series.squeeze()
            sin_fit_series = pd.to_numeric(sin_fit_series, errors='coerce')
    
            # Calculate r² for Sinusoid Fit
            ss_res_sin_fit = np.sum((data_detrend - sin_fit_series) ** 2)
            ...
            all_sin_fit_series.append(sin_fit_series)
    
        # Ensure length match and no empty Series
        for i, s in enumerate(all_sin_fit_series):
            if s.size != len(fit_times):
                all_sin_fit_series[i] = pd.Series(np.nan, index=fit_times)
    
    except:
        all_sin_fits = []
        all_sin_fit_series = []
    
    # ==== Pick Best Fit (without changing order) ====
    if len(all_sin_fits) > 0:
        # Find index of the fit with the highest r2
        best_idx = max(range(len(all_sin_fits)), key=lambda i: all_sin_fits[i]['r2'])
    
        # Only for bookkeeping; this does NOT go into sinusoid()
        for i in range(len(all_sin_fits)):
            all_sin_fits[i]['selected'] = (i == best_idx)
    
        # Prepare parameters for sinusoid (exclude 'r2', 'T_hr_guess', 'selected')
        p0_sin_fit = all_sin_fits[best_idx].copy()
        p0 = {k: v for k, v in p0_sin_fit.items() if k not in ['r2', 'T_hr_guess', 'selected']}
...
#    coords['datetimes']  = arr_times
    ...
    result['tt_sec']            = tt_sec
    result['fit_times']         = fit_times
    result['all_sin_fit_series'] = all_sin_fit_series
